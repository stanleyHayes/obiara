...
func TestViewAuditsBeforeReading(t *testing.T) {
	...
	audit.EXPECT().Append(gomock.Any(), gomock.Any()).DoAndReturn(
		func(_ context.Context, record domain.AccessRecord) error {
			...
			if record.CaseID != "case_1" || record.AgentID != "agent-1" || record.Purpose != domain.PurposeTriage {
				t.Fatalf("record = %#v", record)
			}
			return nil
		})
	cases.EXPECT().FindByID(gomock.Any(), "case_1").Return(
		domain.ReconstituteCase("case_1", "rep_1", "m-2", domain.TierB, domain.QueueTriage, evidenceSvcNow.Add(time.Hour), domain.CaseInReview, "agent-1", 2, evidenceSvcNow, nil),
		nil,
	)
	...
	service := NewEvidenceService(reports, cases, audit, func() time.Time { return evidenceSvcNow }, func() string { return "acc_test" })
	bundle, err := service.View(context.Background(), "case_1", "agent-1", domain.PurposeTriage)
	if err != nil {
		t.Fatal(err)
	}
