...
| Production topology                                   | Mixed: external decision then repository ADR | Architecture owner                 | Backend-only manual Render candidate; no approval to deploy | Blocked                     | Residency decision, provider evidence and an accepted follow-up ADR                   |
...
| Production resource creation and traffic activation   | Production action                            | Platform and release managers      | Configuration only; no resource or traffic evidence         | Blocked                     | Approved topology, credentials, production rehearsal and explicit release authority   |
