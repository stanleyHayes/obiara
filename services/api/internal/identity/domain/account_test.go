...
func TestTierTransitionMatrix(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			account := ReconstituteAccount("id_1", ReconstituteContact(ChannelSMS, "+233550000101"), AccountActive, tc.from, 1, nil, testNow)
...
func TestTransitionRequiresReasonAndActor(t *testing.T) {
	account := ReconstituteAccount("id_1", ReconstituteContact(ChannelSMS, "+233550000101"), AccountActive, TierUnverified, 1, nil, testNow)
...
func TestBlockedAccountCannotTransition(t *testing.T) {
	account := ReconstituteAccount("id_1", ReconstituteContact(ChannelSMS, "+233550000101"), AccountBlocked, TierVerified, 1, nil, testNow)
