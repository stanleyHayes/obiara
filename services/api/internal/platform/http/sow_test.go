...
func TestASowIsSentAsTheSessionAndReportsThatItIsWaiting(t *testing.T) {
	...
	held, err := sowdomain.Accept("sow_1", "actor-key", "target-key", "hello", nil,
	...
	response := sowRequest(t, stub, `{"targetId":"member_2","body":"hello","confirmed":true}`, "cmd-1")
...
func TestAnUnconfirmedSowIsRefusedRatherThanDefaulted(t *testing.T) {
	...
	response := sowRequest(t, stub, `{"targetId":"member_2","body":"hello"}`, "cmd-1")
...
func TestASowWithoutARequestIdNeverReachesTheSeedEconomy(t *testing.T) {
	...
	stub := &sowStub{}
	response := sowRequest(t, stub, `{"targetId":"member_2","body":"hello","confirmed":true}`, "")
	if response.Code != http.StatusUnprocessableEntity {
		t.Fatalf("status = %d, want 422: %s", response.Code, response.Body.String())
	}
	if stub.calls != 0 {
...
func TestAnUnverifiedMemberCannotSow(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/v1/seed/sows", strings.NewReader(`{"targetId":"member_2","body":"hi","confirmed":true}`))
	request.Header.Set("Authorization", "Bearer token")
	request.Header.Set("Content-Type", "application/json")
