...
func openCase(t *testing.T) VerificationCase {
	t.Helper()
	verificationCase, err := NewCase("vc_1", "id_1", "key_1", "0001", testDOB, testNow)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewCaseValidation(t *testing.T) {
	if _, err := NewCase("", "id_1", "key_1", "0001", testDOB, testNow); err != ErrCaseIDRequired {
		t.Fatalf("missing id = %v", err)
	}
	if _, err := NewCase("vc_1", " ", "key_1", "0001", testDOB, testNow); err != ErrAccountIDRequired {
		t.Fatalf("missing account = %v", err)
	}
	if _, err := NewCase("vc_1", "id_1", " ", "0001", testDOB, testNow); err != ErrCardKeyRequired {
