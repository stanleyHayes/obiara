...
func NewModule(ctx context.Context, database *mongo.Database) (Module, error) {
	...
	purposes := make([]domain.Purpose, 0, 4)
	for _, params := range []domain.NewPurposeParams{
		...
		{ID: application.VoiceIntroductionID, Kind: domain.PurposeVoice, Version: application.CurrentVersion, ContentRef: "content.voice.v1", Status: domain.PurposeActive, EffectiveSince: effectiveSince},
	} {
