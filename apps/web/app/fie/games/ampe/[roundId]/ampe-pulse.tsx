    ...
    <main className="ampe">
      ...
      <section className="ampe-stage" aria-labelledby="ampe-title">
        <div className="ampe-meta">
          <span>Round 03</span>
          <span>{roundId.slice(0, 9)}</span>
          <span>Low-data pulse</span>
        </div>
        <div className="ampe-players">
          ...
          <div>
            <span>Y</span>
            <strong>You</strong>
            <small>{state.stage === "ready" ? "Not ready" : "Ready"}</small>
          </div>
        </div>
        <h2 id="ampe-title">
          {state.stage === "ready"
          ...
          <>
            ...
            >
              {(["together", "apart"] as const).map((gesture) => (
                <button
                  aria-pressed={state.choice === gesture}
                  key={gesture}
                  onClick={() => dispatch({ type: "choose", gesture })}
                  type="button"
                >
                  <strong>
        ...
        {state.stage === "locked" ? (
          <div className="ampe-lock">
            <p>Your gesture is encrypted and hidden from Ama.</p>
            <button onClick={() => dispatch({ type: "reveal" })} type="button">
              Simulate both locked · reveal
            </button>
          </div>
        ) : null}
        {state.stage === "reconnecting" ? (
          <div className="ampe-lock">
            <p>No forfeit. Your hidden choice remains held.</p>
            <button
              onClick={() => dispatch({ type: "reconnected" })}
              type="button"
            >
              Reconnect safely
            </button>
        ...
        {state.stage === "revealed" ? (
          <div className="ampe-reveal" role="status">
            ...
            <div>
              <span>You</span>
              <strong>{state.choice}</strong>
            </div>
            <p>
              The beat matched. No public score or profile signal was created.
            </p>
          </div>
        ) : null}
        <footer>
          <button
            disabled={state.stage === "revealed"}
