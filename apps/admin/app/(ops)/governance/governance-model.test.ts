...
describe("market-pack governance", () => {
  it("requires parity, valid placeholders and terminology review", () => {
    ...
    expect(checksPass({ ...initialGovernanceState, translatedKeys: 147 })).toBe(false);
    expect(checksPass({ ...initialGovernanceState, placeholdersValid: false })).toBe(false);
    expect(checksPass({ ...initialGovernanceState, terminologyReviewed: false })).toBe(false);
  });

  it("requires a substantive human acknowledgement", () => {
    const short = governanceReducer(
      governanceReducer(initialGovernanceState, { type: "review-note", value: "looks fine" }),
  ...
  it("requires a distinct second approver", () => {
    ...
    const self = governanceReducer(
      governanceReducer(first, { type: "second-approver", actor: "operator•••A1" }),
      { type: "confirm-second-approval" },
    );
    expect(self.publishState).toBe("first_approved");
    const distinct = governanceReducer(
      governanceReducer(first, { type: "second-approver", actor: "operator•••B8" }),
      { type: "confirm-second-approval" },
    );
