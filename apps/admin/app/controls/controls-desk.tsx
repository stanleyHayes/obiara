    ...
    <Box sx={{ bgcolor: "#f7efe3", color: "#2b151f", minHeight: "100vh", py: 4 }}>
      <Container maxWidth="lg">
        <Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ alignItems: { md: "center" }, justifyContent: "space-between", mb: 5 }}>
          <Box>
            <Typography sx={{ color: "#8e3159", fontSize: 12, fontWeight: 800, letterSpacing: 1.4 }}>RUNTIME CONTROLS</Typography>
            ...
            <Typography sx={{ color: "#69535d", mt: 2 }}>{state.proposalRef} · immutable proposal preview</Typography>
          </Box>
          <Link href="/"><Button variant="outlined">Back to command centre</Button></Link>
        </Stack>

        <Alert severity={state.state === "apply_ready" ? "success" : "warning"} sx={{ borderRadius: 3, mb: 3 }}>
          <strong>{state.state === "apply_ready" ? "Apply-ready; nothing activated." : "No runtime state has changed."}</strong>{" "}
          A separate audited control plane applies approved proposals and verifies rollback.
        </Alert>
        ...
        <Box sx={{ display: "grid", gap: 2, gridTemplateColumns: { xs: "1fr", md: "repeat(4,minmax(0,1fr))" } }}>
          {[
            ...
            <Card key={label} sx={{ borderRadius: 4, p: 2.5 }}>
              ...
              <Typography sx={{ fontSize: 20, fontWeight: 800, mt: 1, overflowWrap: "anywhere" }}>{value}</Typography>
            </Card>
          ))}
        </Box>
        ...
        <Card sx={{ borderRadius: 4, display: "grid", gap: 4, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, mt: 3, p: 3 }}>
          <Box>
            ...
            <Typography component="h2" sx={{ fontSize: 34, fontWeight: 800, mt: 2 }}>Disabling is the safe direction.</Typography>
            <Typography sx={{ color: "text.secondary", lineHeight: 1.6, mt: 1 }}>
              This proposal cannot target a person, widen itself to production, remain forever, deploy code or claim an automatic rollback. Its exact scope and expiry are fixed.
            </Typography>
          </Box>
          ...
          {state.state === "draft" ? (
            ...
            <Box>
              <Alert severity="info" sx={{ mb: 2 }}>Proposed by {state.proposer}</Alert>
              <FormControl fullWidth>
                <InputLabel id="control-approver-label">Distinct second approver</InputLabel>
                <Select
                ...
                >
                  ...
                  <MenuItem value="operator•••C9">Efua · release controller</MenuItem>
                </Select>
              </FormControl>
              <Button
                disabled={!state.secondApprover || state.secondApprover === state.proposer}
                fullWidth
            ...
            <Alert severity="success">
              <strong>Kill-switch proposal is apply-ready.</strong><br />
              Staging/GH only, two-hour expiry, disabled fail-closed posture. No runtime or member state changed.
            </Alert>
          )}
        </Card>
      </Container>
