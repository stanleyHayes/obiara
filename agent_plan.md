...
| S7-024 | E10-S08 tournament, ladder and anti-cheat domain | 8 | `/root/client_quality` | IN PROGRESS | isolated `services/api/internal/games/competition/` domain/application/persistence; no client UI | claimed 2026-07-26 after S7-019 and E10 game/rating/conduct foundations; renumbered around external S7-023 | GoMock, bracket/ladder/fair-play/appeal property/race and live MongoDB Testcontainers proofs pending | Opt-in bounded cohorts; game results only; conduct signals remain separate; anti-cheat creates review evidence not automatic accusation; no matching/discovery/popularity/pay-to-rank export |
...
| S8-001 | E11-S01 AI gateway, vendor policy and audit metadata | 8 | `codex-root` | IN PROGRESS | isolated `services/api/internal/ai/gateway/` domain/application boundary; no vendor network adapter | claimed 2026-07-26 while final E10 server slices run; foundation for all E11 capabilities | GoMock, allowlist/redaction/consent/audit/property/race and persistence applicability proofs pending | No direct client/vendor call; capability/model/vendor allowlist, bounded purpose and data classes, current consent and region policy, fail-closed redaction, immutable minimal audit metadata; no raw prompt/response persistence or silent fallback |
