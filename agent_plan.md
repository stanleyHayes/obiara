...
| S7-004 | E09-S05 Runsheet, timer and game-segment mount points | 5 | `/root/client_quality` | IN PROGRESS | isolated `services/api/internal/fire/runsheet/` bounded context | claimed 2026-07-26 after S6-003 Fire foundation | GoMock, property/race and Mongo Testcontainers host-authority/timer/replay/privacy proofs pending | Versioned ordered segments with server-time transitions; only authorized host/co-host advances; timer is informational and never auto-ejects/punishes; game mount uses allowlisted opaque capability refs |
