...
| S3-002 | E04-S02 First-run interactive walk | 3 | `codex-root` | IN PROGRESS | new `apps/web/app/fie/welcome/` route and client state model | claimed 2026-07-26 after S3-001 | versioned completion tests, typecheck/build and desktop/390 rendered QA | Skippable, no pressure, keyboard/screen-reader/reduced-motion safe; rebase before every commit |
| S3-010 | E05-S01 Circle types, privacy defaults and membership kernel | 3 | `/root/client_quality` | IN PROGRESS | new isolated `services/api/internal/circle/` bounded context | claimed 2026-07-26 after E03 profile/auth foundation | privacy-default, membership, concurrency and Mongo Testcontainers tests | Backend-only lane; deny-by-default circle access; rebase before every commit and hand off without pushing |
