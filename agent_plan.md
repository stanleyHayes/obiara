...
| S3-003 | E04-S03 Compound home and zone indicators | 3 | `codex-root` | IN PROGRESS | new `/fie` member web shell/home route and shared Fie client components | claimed 2026-07-26 after S3-001/S3-002 | shell state tests, typecheck/build and desktop/390 rendered QA | Home, four zones, connection/queued states and tonight's fire discoverable within one viewport; rebase before every commit |
...
| S3-013 | E05-S05 Trust edge model and bounded trust-path projection | 3 | `/root/client_quality` | IN PROGRESS | new isolated `services/api/internal/trust/` bounded context | claimed 2026-07-26 after S3-010 | provenance, visibility, cycle/bounds and property tests | Derived paths only, immutable provenance and no global graph exposure; rebase before every commit and hand off without pushing |
