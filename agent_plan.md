...
| S4-015 | E06-S10 Ember issuance/redemption | 3 | `/root/backend_fires` | IN PROGRESS | `services/api/internal/fire/ember/` | claimed 2026-07-26; dependency E09 fire attendance complete (S6-003) | FR-402 unit + integration tests pending | — |
...
| S6-010 | E08-S08 Harvest/weaver specification and order handoff | 5 | `/root/client_quality` | IN PROGRESS | `services/api/internal/cloth/harvest/` plus `internal/product/cloth-harvest-weaver-spec.md` | specification completed by `codex-root` in `41afabc`; backend handoff reassigned 2026-07-26 after S6-006 | GoMock, property/race and Mongo Testcontainers consent/idempotency/privacy proofs pending | Pair-authorized immutable recipe handoff contains production-safe render tokens and opaque delivery ref only; no raw reflection, address, payment, public listing or vendor access beyond the order |
| S7-001 | E09-S02 LiveKit room and token adapter | 5 | `/root/courtship_closure` | IN PROGRESS | isolated `services/api/internal/realtime/livekit/` outbound adapter and contract | claimed 2026-07-26 alongside E09-S01 Fire foundation | official latest LiveKit dependency verification; GoMock/unit/property tests; contract privacy and expiry proofs pending | Server issues short-lived least-privilege room tokens for opaque participants only; no client API secret, phone/email identity, room browse or recording grant |
| S7-002 | E09-S03 Host stage, mute, eject and co-host controls | 5 | `/root/consent_kernel` | IN PROGRESS | isolated `services/api/internal/fire/control/` bounded context | claimed 2026-07-26 after S6-003 Fire scheduling foundation | GoMock, property/race and Mongo Testcontainers authorization/replay/privacy proofs pending | Server-authoritative least-privilege host/co-host actions; self-unmute remains participant-controlled; eject revokes room access; immutable reason-coded audit with no public accusation |
| S7-003 | E09-S04 client low-bandwidth degradation ladder and captions | 5 | `codex-root` | IN PROGRESS | guarded web/Expo Fire room presentation and connection-state model | claimed 2026-07-26 after S6-003 and shared client connection patterns | model/UI tests; web/mobile typecheck/lint/build; desktop/mobile accessibility/browser QA pending | Explicit video → audio → captions → reconnect ladder preserves safety/leave controls and transcript privacy; user can select a lower mode without shame, urgency or hidden data use |
