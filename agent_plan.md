...
| S8-052 | E15-S08 analytics retention, pseudonymization and aggregation jobs | 8 | `/root/consent_kernel` | IN PROGRESS | isolated `services/api/internal/analytics/retention/` domain/application/persistence boundary; no producer schema, dashboard or source-event creation ownership | claimed 2026-07-27 after S8-030 consent-aware analytics and S8-047 phase-gate closure; Doc 08/09 90-day pseudonymization and 13-month aggregation rules reviewed | GoMock, age-boundary/idempotency/property/race/fuzz and live MongoDB Testcontainers transaction/concurrency/privacy proofs pending | Versioned keyed re-pseudonymization at 90 days, bounded leased batches, 13-month aggregate-before-erasure transaction, immutable processing receipts and explicit aggregate dimensions | No raw member/content recovery, stable cross-epoch identity, early erasure, aggregate loss/double count, unbounded scan, destructive retry, cross-purpose join, ranking/score or dashboard mutation |
...
| S8-050 | E15-S07 quarterly fairness, regret and safety projection | 8 | `/root/client_quality` | IN PROGRESS | isolated `services/api/internal/analytics/fairness/` aggregate domain/application/persistence projection; no admin UI, source analytics/safety mutation or rollout ownership | claimed 2026-07-27 after S8-030 consent-aware analytics, S8-047 P0 projection and safety case lanes completed; Doc 08 quarterly exposure parity/colorism-failure rule and Doc 04 regret/Tier-A gates reviewed | GoMock, exact parity/regret/safety/incomplete-input/property/race/fuzz and live MongoDB Testcontainers concurrency/idempotency/privacy proofs pending | Versioned quarterly aggregate facts expose explicit cohort denominators, bounded exposure parity, colorism drift as an audited failure, regret trend and unresolved Tier-A count; incomplete evidence never passes and every snapshot is immutable | No raw/proxy protected trait or member identity, hidden denominator/tolerance, fairness score for matching/rank, automatic correction/rollout/enforcement, source-event edit, safety-case action, content/free text, cross-purpose reuse or model/vendor decision |
...
| S8-054 | E16-S07 admin feature flags and kill switches | 5 | `codex-root` | IN PROGRESS | `apps/admin/app/controls/` MUI proposal desk with pure immutable fixtures and dashboard entry; no runtime flag, deployment, member or service mutation | claimed 2026-07-27 after E16-S01 stepped-up admin auth closure | admin scope/reason/expiry/kill-switch/distinct-approver and rendered semantic/overflow proofs pending | Versioned proposal exposes exact environment, market, capability, expiry and rollback posture; kill switch fails closed; stepped-up proposer and distinct second approver required for apply-ready preview | No direct runtime mutation, global-by-default scope, permanent temporary flag, same-person approval, silent activation, member targeting, experiment dark pattern, deployment or automatic rollback claim |
| S8-039 | E09-S07 ember-close transaction: fire dims to embers with frozen attendance | 9 | `/root/backend_fires` | DONE | `services/api/internal/fire/` (close flow), close route + OpenAPI | claimed/completed 2026-07-26 | -race integration: non-host rejected (FR-401), host close dims to embers, roster frozen at capacity, admissions rejected after close; domain state tests (host-only, embers not re-closable, live close allowed); Redocly + client regen + drift green | Self-reviewed; waitlisted members are excluded from the frozen roster (going only); cancellation-after-close policy noted for host-controls follow-up |
