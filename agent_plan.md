...
| L2-001 | Final external-gate handoff inventory and claim-safe ledger | Launch 2 | `codex-root` | IN PROGRESS | `agent_plan.md`, launch inventory/checkpoint reconciliation only | claimed 2026-07-27 after Launch 1 closeout | Pending reconciliation of every remaining external gate to an executable handoff | Must leave no repository task implicit while preserving real authority boundaries |
| L2-002 | Founder/legal residency and DPIA decision-record contract | Launch 2 | `/root/security_closure` | IN PROGRESS | isolated decision schema, validator, blocked synthetic fixture and decision packet; no legal conclusion, signature or approval | claimed 2026-07-27 after L1-003 | Pending adversarial policy tests, race/vet and review note | Must reject unsigned, self-approved, stale, synthetic, incomplete-scope and repository-issued legal evidence |
| L2-003 | Provider/procurement diligence evidence contract | Launch 2 | `/root/consent_kernel` | IN PROGRESS | isolated provider-diligence schema/validator/templates covering Atlas, storage/CDN, LiveKit and communications; no provider account, purchase or network call | claimed 2026-07-27 after L1-002 | Pending unit/property tests, strict fixtures and cross-review | Must require DPA, subprocessors, locations, retention/deletion, keys, support, breach, cost and exit evidence per provider |
| L2-004 | Ghana device/network field-test execution kit | Launch 2 | `/root/client_quality` | IN PROGRESS | isolated field-test manifest, validator, runner documentation and synthetic example; no real member data, provider spend or production target | claimed 2026-07-27 after L1-005 | Pending exact device/network/sample/budget and synthetic-boundary proofs | Must cover Android floor, representative 3G/fixed paths, compute/database/media measurements and immutable evidence |
| L2-005 | Credential/store and cohort/operations readiness handoff kit | Launch 2 | `/root/consent_kernel` | READY | isolated metadata-only readiness schemas/templates; no secret, store submission, recruitment, contact or staffing mutation | ready after L2-003 | Pending strict validation and fail-closed external-authority tests | Must cover controlled account custody, signing ceremony, UAT consent/training/completion, circles/hosts and support/T&S coverage |
| L2-006 | Admin external-gate evidence intake surface | Launch 2 | `codex-root` | READY | `apps/admin/app/launch/` and shared admin presentation only; no upload, approval, launch or external mutation | ready after L2-001; contracts cross-reviewed before closure | Pending model/UI tests, typecheck/lint/build and rendered desktop/mobile QA | Codex owns UI; operators can inspect requirements and prepare opaque handoff records but cannot satisfy or approve gates |
| L2-007 | Final repository backlog closure audit and publish | Launch 2 | `codex-root` | READY | repository-wide evidence mapping, quality gates and ledger closeout | ready after L2-002 through L2-006 | Pending full checks, exact contract CLI proofs, rebase and remote parity | Completion means no agent-buildable task remains; production stays blocked until named external authorities act |
