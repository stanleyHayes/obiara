...
| S2-006 | E01-S06 Localization, accessibility and copy-policy lint | 2 | `/root/client_quality` | IN PROGRESS | `packages/config-eslint/`, `packages/quality-client/`, relevant CI configuration | claimed 2026-07-26 after S2-002 | failing fixtures for hardcoded copy, missing keys, bad placeholders and banned pressure patterns; CI proof | 5 SP; documented narrow escape hatch only for non-product data; rebase before every commit |
| S2-007 | E01-S08 Cross-platform loading, empty, error and offline patterns | 2 | `codex-root` | IN PROGRESS | `packages/ui-web/src/states/`, `packages/ui-mobile/src/states/`, state catalogs in `packages/i18n/` | claimed 2026-07-26 after S2-002, S2-003, S2-005 | deterministic web/RN state and assistive-technology tests | 5 SP; include queued, low-bandwidth and permission-denied states; rebase before every commit |
