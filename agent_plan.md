...
| S2-017 | E03-S07 Core profile and privacy controls | 2 | `/root/client_quality` | IN PROGRESS | `services/api/internal/profile/` | claimed 2026-07-26 after S1-002 | profile boundary tests | Isolated backend lane; rebase before every commit and hand off without pushing |
| S2-018 | E03-S04 Voice+face liveness orchestration and uncertainty handling | 2 | `/root/consent_kernel` | IN PROGRESS | `services/api/internal/verification/liveness/` or another non-conflicting bounded subpath | claimed 2026-07-26 after S2-014 completion | liveness adapter contract tests | Coordinate with published S2-015 verification API; rebase before every commit and hand off without pushing |
| S2-019 | E03-S12 Shared-device/collision and known-name review cases | 2 | `/root/mobile_gestures` | IN PROGRESS | new isolated identity collision bounded paths only | claimed 2026-07-26 after S2-011 | collision case tests | Do not overlap existing identity files without a new coordination note; rebase before every commit and hand off without pushing |
| S2-020 | E03-S11 Admin verification queue and evidence access audit | 2 | `codex-root` | IN PROGRESS | `apps/admin/` verification queue UI; generated API client read-only unless a separately coordinated contract gap is proven | claimed 2026-07-26 after S2-015 completion | admin interaction tests, typecheck/build and desktop/390 rendered QA | Privacy-redacted evidence only; deliberate approval/rejection; queue/error/empty/offline states; rebase before every commit |
