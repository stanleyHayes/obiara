...
| S1-009 | E02-S09 Worker scheduling, retries, dead letters and job observability | 1 | `/root/backend_worker` | IN PROGRESS | `services/worker/`, relocated shared packages `internal/platform/` (mongo, outbox, inbox, idempotency) + import updates in `services/api/` | claimed 2026-07-26; dependency S1-004 complete; shared-package relocation required because `services/api/internal` is not importable from `services/worker` | retry/DLQ unit tests + relay integration pending | — |
