...
| S8-066 | E17-S05 Expo EAS build, submit and update release channels | 5 | `codex-root` | IN PROGRESS | `apps/mobile/` EAS/app configuration, release policy and deterministic checks; no EAS project creation, credential provisioning, store submission or OTA publication | claimed 2026-07-27 after E17-S04 release-evidence gates; Expo SDK 57 app config and current official EAS Build/Submit/Update guidance reviewed | preview/staging/production profile isolation, app-version runtime compatibility, store-track/OTA promotion, rollback, signing and production fail-closed proofs pending | Mobile binaries and updates must remain channel/runtime compatible, use bounded non-production distribution, promote the exact tested update, and require explicit production credentials/approval | No cross-channel update, development client in release, embedded secret, automatic store submission, untested OTA, runtime mismatch, mutable version identity, production publish or external account mutation |
