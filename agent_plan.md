...
| S1-002 | E02-S05 MongoDB client, transaction, repository and migration conventions | 1 | `/root/backend_kernel` | BLOCKED | `services/api/internal/platform/mongo/`, Go module files | claimed 2026-07-26 | gofmt/vet/build clean; unit tests green (error translation, migration validation, unreachable-URI Connect); integration suite written but cannot run: Docker Desktop daemon wedged (1 TB Docker.raw, VM slow-booting after founder-approved restart); dependency owner: founder machine Docker recovery | Code complete; blocked only on integration evidence |
| S1-003 | E02-S02 API envelope, OpenAPI, validation and generated TS client | 1 | `/root/backend_http` | IN PROGRESS | `services/api/internal/platform/http/`, `packages/api-client/` | claimed 2026-07-26; dependency S1-001 complete | unit tests + generation drift check pending | — |
...
| S1-005 | E02-S03 Authentication/session/device model | 1 | `/root/backend_identity` | IN PROGRESS | `services/api/internal/identity/` | claimed 2026-07-26; dependency S1-003 complete | unit tests (GoMock) + integration pending Docker recovery | — |
...
| S1-007 | E02-S07 Structured logging, traces, metrics, health and correlation | 1 | `codex-root` + `/root/telemetry` | IN PROGRESS | `services/api/internal/platform/telemetry/`, `packages/observability/` | claimed 2026-07-26 after S1-001 completion | structured logging, trace/metric and PII-redaction tests in progress | Narrow telemetry lane; active S1-002 Mongo, S1-003 HTTP/client and S1-005 identity paths excluded; rebase required before every commit |
