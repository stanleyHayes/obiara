...
| FR-201a | Seed allowance enforced server-side | DONE | LARGE | RECONCILED 2026-09-05: §40 spends a seed on the sprout path; §51 makes seed/sow live, where Acceptance spends it atomically with the write. Two spends now exist on two paths — see §45. (was: I went looking for a spend path under every Akan/seed name and could not find one on the running surface; the ) |
| FR-202 | Sow requires 20s verified playback of Voice of Introduction | DONE | LARGE | RECONCILED 2026-09-05: §39: POST /v1/seed/sprouts refuses an unheard reach; the gate resolves the target's own recordings rather than trusting a caller's asset id. (was: I tried hard to find the gate under another name and could not. What IS built is exactly as the claim describe) |
...
| IM-009 | The Sow is the atomic gesture; aba replaces the swipe and the like | PARTIAL | LARGE | RECONCILED 2026-09-05: §51 makes the sow reachable: POST /v1/seed/sows, gated at Tier 2, held for review, seed spent atomically. It is not yet what replaces the swipe — a released sow reaches nobody until seed/pod is composed (§54). (was: The claim stands. The sow slice is built to depth but is unreachable end to end. Built: services/api/internal/) |
...
| M4-AC-01 | Server-side enforcement of listen gate, allowance, no purchase, 90-day lock | DONE | MEDIUM | RECONCILED 2026-09-05: Listen gate §39, allowance §40, ninety-day lock §42. 'No purchase' holds because nothing sells seeds; it becomes a live constraint the moment a SKU could top up an allowance (§41). (was: The claim's substance holds: no reachable API path enforces the listen gate, the allowance spend, or the 90-da) |
...
| M1-AC-01 | Tier-0 accounts fully gated from member surfaces | DONE | MEDIUM | RECONCILED 2026-09-05 (second pass): §43 closed the deferral §38 recorded. In-circle rooms, stories and games are now gated too, so 'browse yes, participate no' holds for an account that was already a circle member, not only a new one. RECONCILED 2026-09-05: §32 gated the romantic surfaces at Tier 1. Circles, fires listing, profile and settings remain op |
...
| M4-ABUSE-01 | Sentinel screening of sow recordings before delivery, seed refunded on failure | PARTIAL | LARGE | RECONCILED 2026-09-05: §46-§53 deliver screening before delivery and the refund on refusal, with a person deciding every sow. It is human review rather than Sentinel: no automated screen exists, and §49 records why that is the owner's ruling rather than a gap. (was: I went looking for the screened-sow path under every Akan/English name and could not find it on any live route) |
| P0-SENTINEL-SOW-SCREENING | Sentinel v0: sow-screening | PARTIAL | LARGE | RECONCILED 2026-09-05: The policy library is now reachable (§49) with an advisor that offers no opinion and an adjudicator that never claims a human decision, so every sow routes to the queue. Sentinel itself — an actual automated screen — is still absent. (was: Claim stands; I could not refute it. The policy IS fully written and unit-tested: services/api/internal/seed/s) |
...
### What `seed/pod` is actually still waiting on (2026-09-05)

§54 fixed the media reference, and reading the aggregate afterwards narrowed
the rest further than §45 had it. `Play` already refuses a non-recipient;
`change` already refuses a pod that is not active, one that has expired, a
stale revision and a replayed command. So `PlaybackEligibility` is not
carrying revocation, expiry or membership — the aggregate has all of those.

Its one residual job is the consent re-check named in its own comment, and
**under what purpose a sow's recording is played to its recipient has never
been decided.** That is the whole remaining blocker, and it is a product
question rather than an adapter.

Worth saying plainly: the sower's own withdrawal is already modelled — it is
`Revoke`, and the aggregate honours it. So the residual purpose is about the
recipient's side, and inventing one would be inventing policy.

### Audit rows reconciled after the sow chain

| Requirement | Now | Why |
| --- | --- | --- |
| FR-202 | DONE | The listen gate, resolving the target's own recordings. |
| FR-201a | DONE | A sow spends a seed. Note §45: two spends now exist on two paths. |
| M4-AC-01 | DONE | Listen gate, allowance and the ninety-day lock. "No purchase" holds only because nothing sells seeds — §41's promotion work is where that could break. |
| IM-009 | PARTIAL | The sow is reachable, but it does not yet replace the swipe: a released sow reaches nobody until pod is composed. |
| M4-ABUSE-01 | PARTIAL | Screening before delivery and the refund both exist, by human review rather than Sentinel. |
| P0-SENTINEL-SOW-SCREENING | PARTIAL | The policy library is reachable and routes everything to a person; Sentinel itself is still absent. |
