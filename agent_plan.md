...
| S1-003 | E02-S02 API envelope, OpenAPI, validation and generated TS client | 1 | `codex-root` + `/root/api_contract` | DONE | `services/api/internal/platform/http/`, `services/api/openapi/`, `packages/api-client/` | claimed/completed 2026-07-26 after S1-001 completion | Redocly strict OpenAPI validation; deterministic generated-client drift check; 12/12 workspace lint/type/test tasks; 4/4 builds; 3 client tests; Go HTTP/OpenAPI tests with race detector; Go vet; JS audit 0 high/critical | Cross-review fixed eight strict-contract findings and a newly introduced high transitive `js-yaml` advisory; auth is deferred to S1-005, durable replay to S1-004, and duplicate-conflict mapping needs a transport-neutral repository sentinel; active S1-002 paths remained untouched |
