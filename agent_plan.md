...
| S3-004 | E04-S04 Abɔnten public community shell without romantic initiation | 3 | `codex-root` | IN PROGRESS | new `apps/web/app/fie/abonten/` route, interaction model and zone shell primitives | claimed 2026-07-26 after S3-003 | interaction tests, typecheck/lint/build and desktop/390 rendered QA pending | Community moments, fires and notices only; no dating language, match controls or romantic initiation; rebase before every commit |
...
| S3-011 | E05-S02 Host application and institutional verification kernel | 3 | `/root/consent_kernel` | IN PROGRESS | new isolated `services/api/internal/host/` bounded context | claimed 2026-07-26 after S3-010 | application, proof, review, audit and Mongo Testcontainers tests | Evidence refs only, manual uncertainty, no silent institutional pass; rebase before every commit and hand off without pushing |
| S3-012 | E05-S03 Circle invites, request/approval and expulsion workflows | 3 | `/root/mobile_gestures` | IN PROGRESS | new isolated circle workflow subpackage without editing S3-010 files | claimed 2026-07-26 after S3-010 | invite expiry, replay, approval/expulsion and concurrency tests | Privacy-safe opaque invites and one-way audits; rebase before every commit and hand off without pushing |
...
| S3-015 | E05-S06 Member trust-path visibility and privacy-safe explanations | 3 | `/root/client_quality` | IN PROGRESS | isolated trust visibility projection/HTTP contract without editing concurrent Fie client files | claimed 2026-07-26 after S3-013 | authorization, consent withdrawal, bounded explanation, contract and Testcontainers tests pending | Explain only paths the member is authorized to see; no global graph, hidden endpoint or withdrawn-edge disclosure; rebase before every commit and hand off without pushing |
