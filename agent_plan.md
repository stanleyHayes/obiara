...
| S8-052 | E15-S08 analytics retention, pseudonymization and aggregation jobs | 8 | `/root/consent_kernel` | DONE | isolated `services/api/internal/analytics/retention/` domain/application/persistence boundary; no producer schema, dashboard or source-event creation ownership | commit `089a166`; exact age/idempotency and about 461,000 no-early-action fuzz executions; race/vet; live replica-set MongoDB 8.0.13 Testcontainers leases/transactions/idempotency/aggregate-before-delete/privacy proof; `git diff --check` | Versioned 90-day keyed re-pseudonymization, epoch-unlinkable HMAC, max-500 leases, exact 13-calendar-month aggregate-before-erasure transaction, event/month-only aggregates, immutable semantic receipts and current-policy revalidation green | No raw member/content recovery, stable cross-epoch identity, early erasure, aggregate loss/double count, unbounded scan, destructive retry, cross-purpose join, ranking/score or dashboard mutation |
...
| S8-046 | E14-S07 client escrow milestones, payout statement and dispute flow | 5 | `codex-root` | DONE | guarded web/Expo engagement-finance surface with shared pure `@obiara/escrow-engagement`; no ledger, provider, settlement, evidence or dispute mutation | 3 policy and 67 web tests; policy/web/mobile typecheck/lint; web/mobile production builds; rendered dual-evidence/settlement/dispute/escalation/semantic/desktop-overflow QA; `git diff --check` | Immutable funded amount and fee split, named milestones, dual delivery confirmation before settlement preview, bounded payout statement, dispute freeze and opaque Mpanyimfo escalation ref green | No release before evidence, unilateral term change, over-settlement, member transfer, hidden fee, dispute deletion, raw payment/member data, provider call or fake urgency |
| S8-047 | E15-S03 P0 funnel and phase-exit gate projection | 8 | `/root/consent_kernel` | DONE | isolated `services/api/internal/analytics/p0gate/` domain/application/persistence projection; no admin UI, producer schema or event-pipeline ownership | commit `7defc5b`; exact-boundary/fail/incomplete and 1,000-order property tests; race/vet; about 795,000 fuzz executions; live MongoDB 8.0.13 Testcontainers concurrency/idempotency/privacy proof; `git diff --check` | Versioned exact founder gates compute pods-heard 65%, seed-to-sprout 25%, sprout-to-room 35%, weekly-fire 40%, D30 45%, regret strictly down and zero unresolved Tier-A with explicit denominators; incomplete data never passes | No raw content/member identity, mutable source event, hidden denominator, false phase pass, automatic rollout, cross-purpose reuse, ranking/score or admin mutation |
| S8-048 | E15-S06 client suban explanation and appeal view | 5 | `codex-root` | DONE | guarded web/Expo member accountability surface with shared pure `@obiara/suban-explanation`; no event ledger, mark calculation, panel ruling or server mutation | 3 policy and 67 web tests; policy/web/mobile typecheck/lint; web/mobile production builds; rendered event/suppression/decay/appeal/semantic/desktop-overflow QA; `git diff --check` | Every visible mark traces to bounded member-visible events, weight and decay; finding suppression remains explicit; reason-gated appeal creates an opaque pending reference without changing history or deciding outcome | No cached authority, hidden score, raw safety evidence, third-party identity, automatic mark edit, appeal overwrite, ranking/matching effect, pressure or gamified streak |
| S8-049 | E15-S06 member Suban explanation and appeal domain | 8 | `/root/consent_kernel` | DONE | isolated `services/api/internal/suban/explanation/` domain/application/persistence boundary; no mark formula, safety ruling or client UI ownership | commit `ba59ebf`; ownership/redaction/dual-role/audit and 1,000-order property tests; race/vet; about 1.27 million fuzz executions; live MongoDB 8.0.13 Testcontainers concurrency/idempotency/privacy proof; `git diff --check` | Self-authorized bounded explanation uses recomputable marks with generic effects and no numbers; adverse-only appeal preserves its source event and requires distinct authorized human resolution with opaque reasoning ref and immutable CAS history | No other-member view, numeric/hidden score, raw evidence/content, event deletion/edit, automatic mark/ruling change, self-review, matching/rank/visibility export or model/vendor decision |
| S8-050 | E15-S07 quarterly fairness, regret and safety projection | 8 | `/root/client_quality` | DONE | isolated `services/api/internal/analytics/fairness/` aggregate domain/application/persistence projection; no admin UI, source analytics/safety mutation or rollout ownership | commit `ccccfc4`; exact gate and 1,000-order property tests; race/vet; about 201,000 fuzz executions; live MongoDB 8.0.13 Testcontainers concurrency/idempotency/rehydration/malformed-row/privacy proof; repository-wide Go tests/vet; `git diff --check` | Versioned reviewed parity thresholds, minimum-50 opaque cohorts, exact denominators, colorism-drift audited failure, strictly declining regret, zero unresolved Tier-A, current-authority revalidation and immutable semantic reports green; incomplete evidence never passes | No raw/proxy protected trait or member identity, hidden denominator/tolerance, fairness score for matching/rank, automatic correction/rollout/enforcement, source-event edit, safety-case action, content/free text, cross-purpose reuse or model/vendor decision |
| S8-051 | E15-S03+S07 admin phase-gate, fairness, regret and safety dashboard | 5 | `codex-root` | DONE | `apps/admin/app/analytics/` MUI read-only aggregate dashboard with pure fixtures and dashboard entry; no analytics, safety, rollout or policy mutation | 4 analytics and 30 admin tests; admin typecheck/lint/build; rendered threshold/incomplete/note/fail-closed/semantic/desktop-overflow QA; `git diff --check` | Exact P0 gates and quarterly fairness/regret/safety aggregates show explicit numerators, denominators, evidence completeness and fail-closed release state; bounded operator note leaves facts and blocked state unchanged | No raw member/content/protected-trait data, hidden denominator, automatic rollout/correction/enforcement, source edit, ranking score, false pass, micro-cohort or vanity chart |
| S8-053 | E16-S06 admin market-pack and terminology governance | 5 | `codex-root` | DONE | `apps/admin/app/governance/` MUI review desk with pure immutable proposal fixtures and dashboard entry; no i18n registry, deployment or market activation mutation | 3 governance and 33 admin tests; admin typecheck/lint/build; rendered parity/placeholder/human-note/same-person-block/distinct-approval/semantic/desktop-overflow QA; `git diff --check` | Versioned market-pack proposal exposes parity, placeholder and terminology review; substantive human acknowledgement and distinct second approver create only a publish-ready preview while the current registry remains unchanged | No machine translation as truth, silent fallback approval, missing-key publish, placeholder drift, same-person approval, automatic market activation, registry overwrite or culturally unreviewed terminology |
| S8-054 | E16-S07 admin feature flags and kill switches | 5 | `codex-root` | DONE | `apps/admin/app/controls/` MUI proposal desk with pure immutable fixtures and dashboard entry; no runtime flag, deployment, member or service mutation | 3 controls and 36 admin tests; admin typecheck/lint/build; rendered reason/step-up/self-block/distinct-approval/fail-closed/semantic/desktop-overflow QA; `git diff --check` | Versioned proposal exposes exact staging/GH capability, two-hour expiry and fail-closed disabled posture; stepped-up proposer and distinct second approver create only an apply-ready preview | No direct runtime mutation, global-by-default scope, permanent temporary flag, same-person approval, silent activation, member targeting, experiment dark pattern, deployment or automatic rollback claim |
| S8-055 | E16-S06 market-pack and terminology governance backend | 8 | `/root/consent_kernel` | DONE | isolated `services/api/internal/governance/marketpack/` domain/application/persistence boundary; no runtime i18n registry, deployment or admin UI ownership | commit `7ee19b5`; parity/placeholder/terminology/distinct-role and 1,000-order property tests; race/vet; about 588,000 fuzz executions; live MongoDB 8.0.13 Testcontainers concurrency/idempotency/privacy proof; `git diff --check` | Immutable digested versions bind exact current master; complete key and placeholder multiset parity, registered do-not-translate law, professional/community/in-context review, substantive checks, distinct roles, opaque refs, current-master revalidation and publish-ready-only CAS state green | No machine translation as truth, missing/extra key, placeholder drift, same-person approval, silent fallback approval, automatic registry overwrite/deploy/market activation, free-text content store or culturally unreviewed terminology |
...
| S8-068 | E17-S09 security scans, DAST and penetration-test closure | 5 | `/root/security_closure` | IN PROGRESS | `.github/workflows/security.yml`, non-production/static DAST policy and bounded penetration-finding closure evidence; no external target, production scan, secret, account or resource mutation | claimed 2026-07-27 after current CodeQL/dependency workflow and E17 release controls review | deterministic workflow/policy/finding-schema tests, actionlint and repository security checks pending | Safe repository-owned fixtures and declared synthetic staging targets only; unresolved or expired qualifying findings fail release closed with immutable redacted closure evidence | No production or arbitrary-host scan, credential in Git/log/artifact, active exploit, automatic risk acceptance, self-approval, silent finding suppression, false penetration-test claim or external mutation |
| S8-067 | E17-S08 bounded performance, load and cost evidence | 8 | `/root/consent_kernel` | IN PROGRESS | `internal/quality/performance/`, database-backed load fixtures, deterministic budgets and cost model; no external load target, paid provider, production traffic or resource mutation | claimed 2026-07-27 after E17-S01/S02/S03/S04 deployment and operational-control foundations; architecture, release, Render topology and existing API/Mongo paths reviewed | reproducible HTTP/Mongo concurrency, latency/error/allocation/connection bounds, cost-input/formula/sensitivity evidence, property/race/vet and live MongoDB Testcontainers load proofs pending | Local bounded profiles produce machine-readable evidence against explicit budgets; cost calculations use committed assumptions and scenario sensitivity without purchase or production inference | No external spend/traffic, vanity throughput, unbounded soak, production/member data, hidden percentile/sample, benchmark-as-SLA claim, provider quote as fact, unsafe concurrency, resource mutation or false capacity approval |
| S8-071 | E18-S06+S07+S08 admin campus attribution, UAT triage and hypercare command center | 8 | `codex-root` | IN PROGRESS | extend `apps/admin/app/launch/` pure model and MUI operations surface; no ambassador payout/ranking, feedback source, incident, notification, launch or production mutation | claimed 2026-07-27 after readiness/staffing/calendar controls; E18 remaining launch stories and existing incident/analytics safeguards reviewed | quality-gated aggregate attribution, consented UAT cohort/training/feedback triage, hypercare SLO/safety/owner cadence and rendered fail-closed proofs pending | Launch operators see only bounded aggregate attribution and redacted operational evidence; quality/safety/readiness blockers remain explicit and all actions prepare human-owned proposals or triage records | No member/contact list, vanity referral leaderboard, per-ambassador performance score, coercive recruitment, raw feedback/content, automatic payout/action/launch, incident closure, hidden denominator or production mutation |
| S8-070 | E17-S07 backup restore and disaster-recovery rehearsal | 8 | `/root/client_quality` | IN PROGRESS | isolated `internal/platform/drrehearsal/` orchestration and `deploy/atlas/rehearsal/` evidence/runbook; no production/source mutation, Atlas resource creation or external restore execution | claimed 2026-07-27 after E17-S02 restore contract implementation; ADR-0003, data classification, DPIA and Atlas controls reviewed | deterministic GoMock lifecycle/property/race/fuzz proofs and live MongoDB 8.0.13 replica-set Testcontainers source-immutability, isolated-restore, integrity, PITR/RPO/RTO and fail-closed cleanup proofs pending | Exact point-in-time watermark restores only to an isolated target; collection counts/digests/indexes, transactional and audit invariants, RPO ≤5m and RTO ≤60m bind immutable redacted evidence with distinct data-owner/security approvals | No source write/drop, production target, credential/PII evidence, fake restore success, missing PITR proof, RPO/RTO miss, same approver, destructive cleanup, production activation or external resource mutation |
