...
| S7-013 | E10-S04 conduct-only suban integration | 5 | `/root/client_quality` | IN PROGRESS | isolated `services/api/internal/games/conduct/` bounded context; no rating/matching coupling | reassigned 2026-07-26 after S7-011 handoff; previous agent released clean before implementation | GoMock, bounded-conduct/property/race and live MongoDB Testcontainers proofs pending | Suban records only conduct signals from bounded game events; never skill, win/loss, rating, popularity or matching visibility; appeals and provenance remain explicit |
...
| S7-018 | E10-S06 client Anansesɛm relay and publish consent | 5 | `codex-root` | IN PROGRESS | guarded web/Expo private story-relay route with alternating contribution and separately optional publish consent | claimed 2026-07-26 after E10 client foundations; no backend file overlap | model/UI tests; web/mobile typecheck/lint/build; rendered accessibility/interaction/overflow QA pending | Strict alternating relay; drafts stay private by default; publishing requires a separate explicit dual-consent state and never exposes room, contact, matching, trust or private authorship data |
