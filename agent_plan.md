...
| S2-006 | E01-S06 Localization, accessibility and copy-policy lint | 2 | Unassigned | READY | `packages/config-eslint/`, `packages/quality-client/`, relevant CI configuration | Depends S2-002 (done) | failing fixtures for hardcoded copy, missing keys, bad placeholders and banned pressure patterns; CI proof | 5 SP; documented narrow escape hatch only for non-product data |
| S2-007 | E01-S08 Cross-platform loading, empty, error and offline patterns | 2 | Unassigned | READY | `packages/ui-web/src/states/`, `packages/ui-mobile/src/states/`, state catalogs in `packages/i18n/` | Depends S2-002, S2-003, S2-005 (done) | deterministic web/RN state and assistive-technology tests | 5 SP; include queued, low-bandwidth and permission-denied states |
