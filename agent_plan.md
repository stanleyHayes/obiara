...
| L2-008 | Field-test CLI blocked-decision exit semantics correction | Launch 2 | `/root/client_quality` | IN PROGRESS | `internal/quality/fieldtest/cmd/` decision output and process-level tests only; no manifest weakening, device/network operation or external mutation | claimed 2026-07-27 after L2-007 audit reproduced valid blocked manifest exiting 0 without output | Pending deterministic JSON decision, exit-code contract, process/race/vet tests and documentation correction | Valid blocked evidence must emit machine-readable `blocked` and exit 2; exit 0 is reserved for `qualified-field-evidence`; invalid input exits 1 and bad CLI usage exits 64 | No silent success, blocked-as-qualified inference, manifest bypass, production/member data, device/network/provider action, spend or launch approval |
