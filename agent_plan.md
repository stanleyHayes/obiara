...
| S8-004 | E11-S04 consent-controlled matching feature registry | 5 | `/root/client_quality` | IN PROGRESS | isolated `services/api/internal/matching/features/` domain/application/persistence boundary; no model/vendor use | claimed 2026-07-26 after S8-001 and client S8-003 | GoMock, grant/withdraw/snapshot/revalidation/property/race and live MongoDB Testcontainers proofs pending | Versioned allowlisted features only; per-member purpose-bound grants, immediate withdrawal, pair intersection and immutable decision snapshot; no sensitive inference, default-on optional feature, stale grant, raw content or retroactive consent |
| S8-005 | E11-S06 client Okyeame whitelist, disclosure and refusal controls | 5 | `codex-root` | IN PROGRESS | existing guarded web/Expo Okyeame surfaces and pure client policy model; no vendor/network adapter | claimed 2026-07-26 after S8-001; bounded UI lane does not overlap active matching packages | whitelist/refusal/no-hidden-memory reducer tests plus web/mobile quality and rendered browser proofs pending | Explicit AI disclosure; only member-invoked feature help, wording help and navigation; refuse matchmaking decisions, autonomous romance, impersonation, counsel leakage, private evidence access and hidden memory |
