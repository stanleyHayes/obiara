...
| S8-052 | E16-S06 admin market-pack and terminology governance | 5 | `codex-root` | DONE | `apps/admin/app/governance/` MUI review desk with pure immutable proposal fixtures and dashboard entry; no i18n registry, deployment or market activation mutation | 3 governance and 33 admin tests; admin typecheck/lint/build; rendered parity/placeholder/human-note/same-person-block/distinct-approval/semantic/desktop-overflow QA; `git diff --check` | Versioned market-pack proposal exposes parity, placeholder and terminology review; substantive human acknowledgement and distinct second approver create only a publish-ready preview while the current registry remains unchanged | No machine translation as truth, silent fallback approval, missing-key publish, placeholder drift, same-person approval, automatic market activation, registry overwrite or culturally unreviewed terminology |
| S8-039 | E09-S07 ember-close transaction: fire dims to embers with frozen attendance | 9 | `/root/backend_fires` | DONE | `services/api/internal/fire/` (close flow), close route + OpenAPI | claimed/completed 2026-07-26 | -race integration: non-host rejected (FR-401), host close dims to embers, roster frozen at capacity, admissions rejected after close; domain state tests (host-only, embers not re-closable, live close allowed); Redocly + client regen + drift green | Self-reviewed; waitlisted members are excluded from the frozen roster (going only); cancellation-after-close policy noted for host-controls follow-up |
