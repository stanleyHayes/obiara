...
| S2-016 | E03-S10 Export, deletion, pause and legal-hold workflows | 2 | `/root/backend_privacy` | IN PROGRESS | `services/api/internal/privacy/`, privacy HTTP routes + OpenAPI | claimed 2026-07-26 | unit + integration + contract tests pending | — |
...
| S2-021 | E03-S08 Voice of Introduction capture/upload/transcription consent kernel | 2 | `/root/consent_kernel` | IN PROGRESS | new isolated `services/api/internal/introduction/` bounded context | claimed 2026-07-26 after S1-010 and S2-012 | consent, media-lifecycle, idempotency and transcription-state tests | No raw transcript or audio leaves consent scope; rebase before every commit and hand off without pushing |
| S2-022 | E03-S09 Doorway Question and photo vault/veil kernel | 2 | `/root/client_quality` | IN PROGRESS | new isolated `services/api/internal/doorway/` bounded context | claimed 2026-07-26 after S1-010 and S2-017 | visibility, vault, veil and retention tests | Media references only, deny-by-default access; rebase before every commit and hand off without pushing |
| S2-023 | E03-S11 Admin verification queue and evidence access audit contract | 2 | `/root/mobile_gestures` | IN PROGRESS | new verification admin HTTP/application subpaths plus OpenAPI/client only where needed | claimed 2026-07-26 after S2-015 and S2-020 | authz, redaction, evidence-access audit and contract tests | Coordinate shared OpenAPI/HTTP paths before edits; rebase before every commit and hand off without pushing |
| S2-024 | E03 client identity and verification onboarding flows | 2 | `codex-root` | IN PROGRESS | new member web onboarding routes/components and client-only state models | claimed 2026-07-26 after S2-011 through S2-020 | interaction tests, typecheck/build, desktop/390 rendered QA | Consent-first, no raw card persistence in client state, explicit uncertainty/manual-review states; rebase before every commit |
