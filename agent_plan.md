...
| S8-002 | E11-S02 rules and trust-path cold-start introductions | 8 | `/root/consent_kernel` | IN PROGRESS | isolated `services/api/internal/matching/coldstart/` domain/application boundary; no model/vendor use | claimed 2026-07-26 after trust/introduction foundations and S8-001 | GoMock, rules/path/privacy/explainability/property/race and persistence applicability proofs pending | Candidate generation uses explicit reciprocal preferences plus privacy-scoped trust paths only; deterministic reason codes, no global browsing, popularity, game skill, inferred sensitive traits, raw path exposure or model ranking |
| S8-003 | E11-S03 client grounded resonance explanation and feature consent | 5 | `codex-root` | IN PROGRESS | guarded web/Expo introduction explanation and consent controls using reason-code fixtures; no backend files | claimed 2026-07-26 alongside S8-002 with no file overlap | model/UI tests; web/mobile typecheck/lint/build; rendered accessibility/interaction/overflow QA pending | Plain-language “why this introduction,” exact controllable features and withdraw path; no destiny/compatibility certainty, hidden score, attractiveness ranking, urgency, popularity, raw trust path or silent AI feature use |
| S7-026 | E12-S02 Tiered T&S queues and SLA routing | 10 | `/root/backend_safety` | IN PROGRESS | `services/api/internal/safety/` (cases), `services/worker/internal/jobs/safety/` | claimed 2026-07-26; continues S7-023 intake; no safety/ overlap with active lanes | SLA deadline + queue integration pending | — |
