...
| S1-013 | S1-007 follow-up: bind telemetry kernel and approved exporter in API/worker composition roots | 1 | `codex-root` | IN PROGRESS | API/worker composition roots, HTTP correlation bridge, deployment telemetry config | claimed 2026-07-26 after API/worker roots and Render topology stabilized | runtime smoke proves correlated log, trace and RED/USE metric without PII; full Go test/vet pending | Use vendor-neutral OTLP/HTTP with fail-closed config, bounded attributes and graceful exporter shutdown |
...
| S4-004 | E06-S03 Server-verified 20-second listening eligibility | 4 | `/root/backend_seed` | IN PROGRESS | `services/api/internal/seed/listening/`, listening HTTP routes + OpenAPI | claimed 2026-07-26; VoI kernel (S2-021) complete, no overlap with active seed/allowance + seed/source lanes | interval-merge property tests + integration pending | — |
| S4-005 | S2-016 follow-up: wire privacy processor into worker with cross-context export/erasure | 4 | `/root/client_quality` | IN PROGRESS | `services/worker/internal/jobs/`, `services/api/internal/privacy/` | claimed 2026-07-26; depends S2-016 (done), S1-009 (done) | export archive assembled, erasure executes via worker job; GoMock + Testcontainers and full worker/API verification pending | Reassigned after independent Doorway review found the already-published `services/api/internal/profile/` implementation satisfies S2-022; avoid a duplicate doorway bounded context |
| S4-006 | E06-S04 Sow composer, media screening and deliberate send | 5 | `/root/consent_kernel` | IN PROGRESS | isolated `services/api/internal/seed/sow/` bounded context; no shared composition files | claimed 2026-07-26 after S4-001/S4-002 and client deliberate-send model | GoMock, race/property and Mongo Testcontainers proofs pending | Screen before delivery; idempotent deliberate command; spend allowance only on confirmed server acceptance; no paid/boost/bypass path |
| S4-007 | E06-S05 Pod delivery, cap and privacy-safe playback | 5 | `/root/mobile_gestures` | IN PROGRESS | isolated `services/api/internal/seed/pod/` bounded context; no shared composition files | claimed 2026-07-26 after S4-001/S4-002 | GoMock, race/property and Mongo Testcontainers proofs pending | Enforce recipient cap and opaque media/member refs; consent/revocation/expiry revalidated on playback; no global queue or reverse discovery |
