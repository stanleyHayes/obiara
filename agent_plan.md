...
| S8-064 | E17-S03 provider-neutral secret management and rotation | 8 | `/root/consent_kernel` | IN PROGRESS | `deploy/secrets/`, shared `internal/platform/secrets/`, API/worker runtime configuration and deterministic repository checks; no provider account, vault value or external resource mutation | claimed 2026-07-27 after E17-S01 environment matrix; SRS NFR-302, data classification, threat model, Render Blueprint, runtime config and CI/security workflows reviewed | secret inventory/ownership/least-privilege/rotation/overlap/revocation policy, runtime fail-closed validation, leak/age/blueprint deterministic checks and focused/race/vet proofs pending | Provider-neutral metadata inventory names every runtime secret, service scope, owner, maximum age, overlap/reload/revocation procedure and evidence; staging/production reject missing or stale secret metadata without logging values | No secret value or digest in Git/tests/logs, shared service credential, build-time secret, silent indefinite overlap, rotation without rollback/revocation/evidence, provider lock-in, production provisioning or external resource mutation |
| S8-065 | E17-S04 CI/CD release evidence and preview/staging/production promotion controls | 5 | `codex-root` | IN PROGRESS | `.github/workflows/`, `deploy/render/` promotion policy and deterministic repository checks; no Render resource creation, external deployment or production activation | claimed 2026-07-27 after E17-S01 staging Blueprint; current CI/security workflows, environment matrix and residency/legal gates reviewed | exact-main-SHA release evidence, checks-pass staging, bounded manual preview, immutable artifact manifest, production approval/gate and rollback proofs pending | Promotion tooling must bind evidence to the exact tested commit, keep preview and staging non-production, and fail closed before production unless every named approval and residency/legal control is current | No untested deployment, mutable/latest tag, automatic production, production data in preview/staging, bypassed environment approval, secret in artifact/log, false deployment claim or external resource mutation |
